...
function ensureSettings() {
  ...
  data.settings.praiseMode = data.settings.praiseMode ?? 'normal';
  data.settings.roleMode = data.settings.roleMode ?? 'englishTeacher';
}
...
studyForm?.addEventListener('submit', (event) => {
  ...
  formData.roleMode = data.settings.roleMode;

  const { item, metaLog } = addStudyItem(data, formData);
  const praise = getPraise(item, metaLog, data.settings.praiseMode);
